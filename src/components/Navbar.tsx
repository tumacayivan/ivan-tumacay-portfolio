...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      <div className="bg-[hsl(var(--ink-charcoal))] text-[hsl(var(--paper))] font-blackops text-[13px] sm:text-sm tracking-[0.3em] py-1 px-4 flex items-center justify-between">
      ...
      <div className="bg-paper-cream border-b-2 border-[hsl(var(--ink-charcoal))] paper-grain relative">
        <div className="w-full px-6 sm:px-10 lg:px-16 xl:px-20 flex items-center justify-between h-14 sm:h-16 relative">
          <a href="#" className="flex items-center gap-3 group">
            <div className="hidden sm:flex flex-col items-end leading-none border-r-2 border-[hsl(var(--ink-charcoal))] pr-3">
              <span className="font-courier text-[13px] tracking-widest text-ink-brown">FILE NO.</span>
              <span className="font-blackops text-base text-ink">IT-2026</span>
            </div>
            <span className="font-blackops text-xl sm:text-3xl tracking-[0.15em] text-ink uppercase">
              IVAN TUMACAY
            </span>
            <span className="hidden md:inline font-courier text-[13px] tracking-widest text-ink-brown border border-dashed-ink px-2 py-0.5">
              SUBJECT
            </span>
          </a>
            ...
            <div className="w-full px-6 sm:px-10 lg:px-16 xl:px-20 py-5 flex flex-col gap-3">
              {navItems.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="flex items-center justify-between font-stencil text-base tracking-[0.18em] text-ink uppercase hover:text-ink transition-colors border-b border-dashed-ink pb-2"
                  onClick={() => setMobileOpen(false)}
                >
                  <span>{item.label}</span>
                  <span className="font-courier text-[13px] text-ink-brown">{item.code}</span>
                </a>
              ))}
              <a
                href="#contact"
