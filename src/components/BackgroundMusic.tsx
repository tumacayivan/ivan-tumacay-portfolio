...
const BackgroundMusic = () => {
  ...
  return (
    ...
    >
      {showAsOn ? <Volume2 className="w-4 h-4 text-ember" /> : <VolumeX className="w-4 h-4" />}
      {/* Level meter: moves while the track is audible, flat when muted */}
      <span aria-hidden className="flex items-end gap-[3px] h-4">
        {[0.9, 0.5, 1, 0.7].map((h, i) => (
          <span
            key={i}
            className="w-[3px] bg-ember origin-bottom"
            style={{
              height: `${h * 100}%`,
